...
🧭 2️⃣ 특수 매개변수 (Special Parameters)

이 변수들은 주로 스크립트의 실행 및 인자(argument)와 관련한 정보를 제공합니다.

| 변수         | 설명                                                             | 예시                                                               |
| ------------ | ---------------------------------------------------------------- | ------------------------------------------------------------------ |
| `$0`         | 스크립트 파일의 이름을 나타냅니다.                               | `myscript.sh`를 실행하면, `$0`는 `myscript.sh`가 됩니다.           |
| `$1`~`$9`    | 스크립트에 전달된 첫 번째부터 아홉 번째까지의 인자를 나타냅니다. | `./script.sh arg1 arg2`에서 `$1`은 `arg1`, `$2`는 `arg2`가 됩니다. |
| `${10}` 이상 | 10번째 이후의 인자를 나타냅니다. 중괄호로 감싸야 합니다.         | `${10}`는 10번째 인자를 의미합니다.                                |
| `$#`         | 스크립트에 전달된 인자의 총 개수를 나타냅니다.                   | `./script.sh a b c`에서 `$#`는 `3`이 됩니다.                       |
| `$*`         | 모든 인자를 하나의 문자열로 묶어서 반환합니다.                   | `./script.sh a b`에서 `"$*"`는 `"a b"`가 됩니다.                   |
| `$@`         | 모든 인자를 각각의 문자열 요소로 이루어진 배열로 반환합니다.     | `./script.sh a b`에서 `"$@"`는 `"a" "b"`가 됩니다.                 |
| `$$`         | 현재 셸 또는 스크립트의 프로세스 ID(PID)를 나타냅니다.           | -                                                                  |
| `$?`         | 마지막으로 실행된 명령어의 종료 상태(exit status)를 나타냅니다.  | `0`은 성공, `0`이 아닌 값은 오류를 의미합니다.                     |
| `$!`         | 가장 최근에 백그라운드에서 실행된 명령어의 PID를 나타냅니다.     | -                                                                  |
| `$-`         | 현재 셸에 설정된 옵션 플래그를 나타냅니다.                       | -                                                                  |
| `$_`         | 마지막으로 실행된 명령어의 마지막 인자를 나타냅니다.             | -                                                                  |

🧭 3️⃣ 환경 변수 (Environment Variables)

이 변수들은 시스템 전반에 걸쳐 설정되며, 셸과 프로세스가 동작하는 환경에 영향을 줍니다.

| 변수           | 설명                                                 |
| -------------- | ---------------------------------------------------- |
| `HOME`         | 현재 사용자의 홈 디렉터리 경로입니다.                |
| `PATH`         | 실행 가능한 파일(명령어)을 찾을 디렉터리 목록입니다. |
| `USER`         | 현재 사용자의 이름입니다.                            |
| `SHELL`        | 현재 사용 중인 셸의 경로입니다.                      |
| `PWD`          | 현재 작업 디렉터리의 경로입니다.                     |
| `BASH_VERSION` | 현재 사용 중인 Bash의 버전 정보입니다.               |
| `HOSTNAME`     | 현재 시스템의 호스트 이름입니다.                     |

---

---

## 🍎 macOS 호환성 (macOS Compatibility)

이 프로젝트는 본래 Windows Git Bash 환경을 기준으로 작성되었으나, macOS (Darwin) 환경에서도 원활하게 동작하도록 호환성 패치가 적용되었습니다.

### 주요 변경 사항

1.  **`utils.sh` 헬퍼 함수 추가**:

    - macOS의 BSD 기반 명령어와 Linux(GNU) 명령어의 차이를 처리하기 위해 공통 헬퍼 함수를 도입했습니다.
    - `sed_i`: `sed -i` (Linux) vs `sed -i ''` (macOS) 차이 해결.
    - `sed_i_bak`: `sed -i.bak` (Linux) vs `sed -i .bak` (macOS) 차이 해결.
    - `date_offset`: `date -d` (Linux) vs `date -v` (macOS) 날짜 계산 차이 해결.
    - `touch_d`: `touch -d` (Linux) vs `touch -t` (macOS) 타임스탬프 변경 차이 해결.

2.  **스크립트 수정**:
    - `lesson12.sh`: GNU sed 전용 플래그인 `/e` (실행) 옵션을 제거하고, 셸 루프를 사용하여 호환성을 확보했습니다.
    - `lesson13.sh`, `lesson26.sh`, `lesson27.sh`, `lesson31.sh`: 위에서 정의한 헬퍼 함수를 사용하여 `sed`와 `date` 명령어가 macOS에서도 정상 동작하도록 수정했습니다.

이제 macOS 터미널(zsh, bash)에서도 별도의 수정 없이 바로 실습을 진행하실 수 있습니다! 🚀
